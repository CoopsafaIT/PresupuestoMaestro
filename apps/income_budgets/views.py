...
def income_budget_register(request):
    if request.method == 'POST':
        if request.POST.get('method') == 'load-excel-file':
            try:
                ...
                for item in range(1, number_rows):
                    ...
                    upd_record = Presupuestoingresos.objects.get(
            ...
            except Presupuestoingresos.DoesNotExist as e:
                messages.warning(
                    request,
        ...
        if request.POST.get('method') == 'recalculate':
            try:
                ...
                qs = Presupuestoingresos.objects.filter(
                    codpresupuestoingresos__in=ids
                )
    ...
    query = (
        ...
        "cc.CodigoCuentaContable= c.CuentaPadre WHERE c.CodTipoCuenta= 3 and c.Nivel=7"
    )
    ...
    if (
        ...
        total = Presupuestoingresos.objects.filter(
            codcentrocostoxcuentacontable_new__codcuentacontable__cuentapadre=account,
            periodo=period
        ).aggregate(
            total_ejecutado_diciembre=Sum('ejecutadodiciembre'),
    ...
    return render(request, 'income_budget_register.html', ctx)


@login_required()
def income_budget_generate_excel_file(request, period, account):
    qs = Presupuestoingresos.objects.filter(
        codcentrocostoxcuentacontable_new__codcuentacontable__cuentapadre=account,
        periodo=period
    )
